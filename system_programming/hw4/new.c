...
void *compute(int *num) {
    int tRecID = *num;
    int startnumber = thread_rec[tRecID]->start;
    ...
    while(checkedblock < block_count) {
        ...
        if (tested(blockID, bitmap)) {
            thread_rec[tRecID]->skipped+=blocksize;
        }
        else {
            int start = blockID * blocksize;
            int end = start + blocksize;
            int i;
            thread_rec[tRecID]->curblock = blockID;
            if (end > max + 1) end = max + 1;
            for (i = start; i < end; i++) {
                totaltest += 1;
                thread_rec[tRecID]->tested+=1;
                if (isperfect(i)) {
                    pthread_mutex_lock(&mtx);
                    ...
                    pthread_mutex_unlock(&mtx);
                    perfect[perfid][0] = i;
                    perfect[perfid][1] = thread_rec[tRecID]->id;
                }
                pthread_mutex_lock(&mtx);
                while (thread_rec[tRecID]->idle) {
